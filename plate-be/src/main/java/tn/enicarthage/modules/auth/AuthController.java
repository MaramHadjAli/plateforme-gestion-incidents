...
// // Since we are mocking the Spring Security AuthManager for brevity while
// giving real production-ready JWT output:
// // In a real scenario, you use `authenticationManager.authenticate(...)`

// // MOCK AUTHENTICATION CHECK
// if (!"admin".equals(loginRequest.getUsername()) &&
// !"tech".equals(loginRequest.getUsername())) {
// return ResponseEntity.status(401).body(Map.of("message", "Invalid
// credentials"));
// }

// String role = "admin".equals(loginRequest.getUsername()) ? "ROLE_ADMIN" :
// "ROLE_TECHNICIAN";

// // Generate Token
// String token = jwtService.generateToken(loginRequest.getUsername(), role);

// AuthResponse response = new AuthResponse(token, loginRequest.getUsername(),
// role);
